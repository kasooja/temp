...
def retrieve_mat_as_coo(matfn, min_size=None):
    ...
    cdef cnp.ndarray[cnp.int32_t, ndim=1] row = np.empty(size, dtype=np.int32)
    cdef cnp.ndarray[cnp.int32_t, ndim=1] col = np.empty(size, dtype=np.int32)
    cdef cnp.ndarray[cnp.float64_t, ndim=1] data = np.empty(size, dtype=np.float64)
    cdef int i = 0
    while not feof(fin):
