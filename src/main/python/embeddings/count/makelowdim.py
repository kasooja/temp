...
def worker(proc_num, queue, out_dir, in_dir, count_dir, words_dic, dim, num_words, min_count=100):
    while True:
        if queue.empty():
            break
        year = queue.get()
        ...
        overall_words = words_dic[year][:num_words]
        print "overall words in year " + str(year) + ": " + str(len(overall_words))
        words = list(valid_words.intersection(overall_words))
        print "Final words in year " + str(year) + ": " + str(len(words))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-words", type=int, help="Number of words to include", default=280594)
    ...
    parser.add_argument("--min-count", type=int, default=11)
