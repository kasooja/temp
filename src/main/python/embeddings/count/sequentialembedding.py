...
from embedding import Embedding, SVDEmbedding
...
class SequentialEmbedding:
    ...
    def get_seq_neighbour_dic(self, word, n=3):
        year_neighbours_dic = collections.OrderedDict()
        for year in self.embeds.keys():
            neighbour_set = set([])
            embed = self.embeds.get(year)
            closest = embed.closest(word, n=n)
            for _, neighbour in closest:
                neighbour_set.add(neighbour)
